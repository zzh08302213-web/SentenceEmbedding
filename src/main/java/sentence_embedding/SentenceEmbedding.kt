...
class SentenceEmbedding {
    ...
    suspend fun init(
        ...
        val maxLengthFromModel =
            maxSeqLengthOverride
                ?: (ortSession.inputInfo["input_ids"]?.info as? TensorInfo)?.shape?.getOrNull(1)
                    ?.let {
        ...
        hfTokenizer = HFTokenizer(tokenizerBytes, maxLengthFromModel)
